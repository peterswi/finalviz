        ...
        svg2.append('text')
            ...
            let stateHover = svg.selectAll('path')
              .on("mouseenter", (event, nodes) => {
                  let d = data
                  d3.select(event.currentTarget).style('fill-opacity', 0.7) // highlight the state being hovered
              ...
              .on("mouseleave", (event, nodes) => {
                d3.select(event.currentTarget).style("stroke-width", "5")//.style('fill-opacity', 1)
                svg2.selectAll("path")
                  .filter(function() {
